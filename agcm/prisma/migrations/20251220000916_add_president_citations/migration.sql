-- CreateTable
CREATE TABLE IF NOT EXISTS "president_citations" (
    "id" TEXT NOT NULL,
...
-- CreateIndex
CREATE INDEX IF NOT EXISTS "president_citations_is_published_ordre_idx" ON "president_citations"("is_published", "ordre");

-- CreateIndex
CREATE INDEX IF NOT EXISTS "president_citations_mandat_id_idx" ON "president_citations"("mandat_id");
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'president_citations_president_id_fkey'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'president_citations_mandat_id_fkey'
    ) THEN
